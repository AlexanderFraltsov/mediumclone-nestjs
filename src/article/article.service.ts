...
export class ArticleService {
	...
	async findAll(currentUserId: number, query: any): Promise<ArticlesResponseInterface> {
		...
		if (query.favorited) {
			...
			if (ids.length > 0) {
				...
				// Необходимо вернуть пустой массив, поэтому передается условие, которое никогда не будет выполнено
		...
		const articlesWithFavorites = articles.map(
			article => ({ ...article, favorited: favoriteIds.includes(article.id) }),
		);
	...
	async removeArticleFromFavorites(slug: string, currentUserId: number): Promise<ArticleEntity> {
		const article = await this.findBySlug(slug);
		const user = await this.userRepository.findOne(currentUserId, {	relations: ['favorites'] });

		const deletedArticleIndex = user.favorites.findIndex(articleInFavorites => articleInFavorites.id === article.id);
		if (deletedArticleIndex !== -1) {
			user.favorites.splice(deletedArticleIndex, 1);
			article.favoritesCount -= 1;
			await this.userRepository.save(user);
			await this.articleRepository.save(article);
		}
